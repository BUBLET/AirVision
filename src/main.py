...
def process_frame(
    ...
    """
    Выполняет основную обработку кадра: извлечение ключевых точек, матчинг, вычисление матриц и проверку угла
    ...
    Параметры:
        ...
        - curr_keypoints - текущие ключ точки
        - curr_descriptors - текущие дескрипторы
        - R, t
    ...
    return curr_keypoints, curr_descriptors, R, t
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        while True:
            ...
            curr_keypoints, curr_descriptors, R, t = result
            
            if not initialization_completed:
                keyframes.append((frame_idx, curr_keypoints, curr_descriptors))
                ...
                visible_map_points, map_point_indices = odometry_calculator.visible_map_points(
                    map_points, curr_keypoints, last_pose
                )
